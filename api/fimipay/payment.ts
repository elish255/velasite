import { handleFimipayPayment } from "../../src/lib/fimipay-handlers.server";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    await sendResponse(res, await handleFimipayPayment(webRequest));
  } catch (error) {
    console.error("/api/fimipay/payment error", error);
    res.statusCode = 500;
