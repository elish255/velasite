import { handleFimipayWithdrawal } from "../../src/lib/fimipay-handlers.server";
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  try {
    ...
    await sendResponse(res, await handleFimipayWithdrawal(webRequest));
  } catch (error) {
    console.error("/api/fimipay/withdrawal error", error);
    res.statusCode = 500;
