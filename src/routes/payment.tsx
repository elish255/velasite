...
export const Route = createFileRoute("/payment")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Lipia activation fee ya 1Vela kwa njia salama ya mobile money." },
    ],
  }),
...
function PaymentPage() {
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(async () => {
      try {
        ...
        if (!token) return;
        const response = await fetch("/api/fimipay/payment-status?requestId=" + encodeURIComponent(request.id), {
          method: "GET",
          headers: { Authorization: `Bearer ${token}`, Accept: "application/json" },
        });
        const data = await readApiResponse(response) as { payment?: PaymentRequest; error?: string };
        if (response.ok && data.payment) setRequest(data.payment);
      } catch {
        // A temporary status-check failure should not interrupt the payment page.
      }
    }, 5000);
  ...
  async function startPushPayment() {
    ...
    try {
      ...
      const response = await fetch("/api/fimipay/payment", {
        method: "POST",
        headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
        body: JSON.stringify({ action: "create", phone: cleanedPhone }),
      });
      const data = await readApiResponse(response) as { request?: PaymentRequest; checkoutUrl?: string | null; error?: string; message?: string };
    ...
    <div className="min-h-screen bg-background pb-10">
      <header className="header-surface px-5 py-5 text-header-foreground">
        <div className="mx-auto flex max-w-2xl items-center gap-3">
          ...
          <div><p className="font-extrabold">1Vela</p><p className="text-xs opacity-80">Automatic Payment</p></div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-2xl px-5 py-7">
        <div className="text-center">
          ...
          <p className="mt-2 text-muted-foreground">Weka namba yako ya mobile money. Utapokea ombi la malipo moja kwa moja kwenye simu yako.</p>
        </div>

        <section className="mt-7 rounded-3xl border border-border bg-card p-6 shadow-card">
          <div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-2xl bg-brand-tint text-primary"><Smartphone className="size-5" /></div><div><h2 className="font-display text-xl font-extrabold">Automatic Payment</h2><p className="text-sm text-muted-foreground">Ombi la malipo litatumwa kwenye simu yako</p></div></div>
          ...
          <button type="button" onClick={() => void startPushPayment()} disabled={submitting || request?.status === "pending" || request?.status === "approved"} className="brand-gradient mt-5 flex w-full items-center justify-center gap-2 rounded-2xl px-6 py-4 text-lg font-extrabold text-brand-foreground shadow-brand disabled:cursor-not-allowed disabled:opacity-60">
            ...
            {request?.status === "pending" ? "Ombi Limetumwa / Inasubiri" : "Lipa Sasa"}
          </button>
        </section>
        ...
        {request && (
          <section className="mt-5 rounded-3xl border border-border bg-card p-6 shadow-card">
            ...
            <div className="mt-5 grid gap-2 text-sm"><InfoRow label="Amount" value={`TZS ${Number(request.amount).toLocaleString("en-US")}`} /><InfoRow label="Phone" value={request.phone} /><InfoRow label="Payment status" value={request.provider_status || "waiting"} /><InfoRow label="Reference" value={request.provider_reference || "—"} /></div>
            {request.provider_checkout_url && <a href={request.provider_checkout_url} target="_blank" rel="noreferrer" className="mt-5 flex items-center justify-center gap-2 rounded-2xl border border-primary px-5 py-3 font-extrabold text-primary">Open Payment <ExternalLink className="size-4" /></a>}
            {providerPaid && request.status === "pending" && <div className="mt-5 rounded-2xl bg-brand-tint p-4 text-sm font-bold text-primary">Malipo yamepokelewa. Admin ata-review na ku-activate account yako.</div>}
        ...
        <p className="mt-5 text-center text-xs font-semibold text-muted-foreground">Usishiriki PIN yako. Ombi la malipo litatokea kwenye simu yako na uta-confirm wewe mwenyewe.</p>
      </main>
    </div>
  );
}
...
async function readApiResponse(response: Response): Promise<unknown> {
  ...
  try {
    ...
    throw new Error(response.ok ? "Server returned an invalid response." : "Payment service is temporarily unavailable. Please try again.");
  }
}
